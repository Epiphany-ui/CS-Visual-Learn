...
import { paths, type StudyItem, type Chapter, type StudyPath, getKnowledgeNameBySlug } from '@/config/studyPaths'

const router = useRouter()
const { username, token, isLoggedIn } = useCurrentUser()
...
const knowledgeWorks = ref<any[]>([])
const knowledgeWorksLoading = ref(false)
const forkingWorkId = ref<Set<number>>(new Set())
const activeWorkVideo = ref<number | null>(null)
...
function openWiki(item: StudyItem, pathId: string, chId: string) {
  ...
  fetchKnowledgeWorks(item.wikiSlug)
...
async function fetchKnowledgeWorks(slug: string) {
  if (!slug) { knowledgeWorks.value = []; return }
  knowledgeWorksLoading.value = true
  try {
    const res = await fetch(`/api/v1/gallery/by-knowledge?slug=${encodeURIComponent(slug)}&size=50`)
    const data = await res.json()
    const list: any[] = data.data?.list || []
    knowledgeWorks.value = list.map((w: any) => ({
      ...w,
      id: w.workId,
      ...
      _likes: w.likeCount || 0,
    }))
  } catch { knowledgeWorks.value = [] }
  finally { knowledgeWorksLoading.value = false }
}
...
function onWikiGenerate(_promptTitle: string) {
  ...
  const slug = activeWikiSlug.value
  const pId = activePathId.value
  activeWikiKey.value = ''
  ElMessage.success('开始生成动画...')
  router.push({ path: '/sandbox', query: { prompt, wikiSlug: slug, pathId: pId || undefined } })
}
...
// ========== 同学作品 Fork ==========
async function handleKnowledgeFork(work: any) {
  if (!isLoggedIn.value) {
    router.push('/login?redirect=' + encodeURIComponent(router.currentRoute.value.fullPath))
    return
  }
  forkingWorkId.value.add(work.id)
  try {
    const res = await fetch(`/api/v1/work/fork?workId=${work.id}`, {
      method: 'POST',
    ...
    if (data.code === 200 && data.data?.sourceCode) {
      ...
      sessionStorage.setItem('cs:fork-source-id', String(work.id))
  ...
  finally { forkingWorkId.value.delete(work.id) }
}

function toggleWorkVideo(work: any) {
  activeWorkVideo.value = activeWorkVideo.value === work.id ? null : work.id
}
</script>
              ...
              <div v-if="knowledgeWorks.length > 0" class="kw-grid">
                <div v-for="work in knowledgeWorks" :key="work.id"
                  ...
                  <div class="kw-info">
                    ...
                    <el-button size="small" round class="kw-fork-btn" @click.stop="handleKnowledgeFork(work)" :loading="forkingWorkId.has(work.id)">
                      🔗 Fork
                    </el-button>
                  </div>
                </div>
