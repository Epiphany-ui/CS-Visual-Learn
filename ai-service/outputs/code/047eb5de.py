...
class ParabolicMotion(Scene):
    def construct(self):
        # 设置场景背景
        background = Rectangle(width=10, height=6, color=WHITE)
        self.add(background)
        ...
        # 播放动画，持续时间为2秒
        self.play(background.animate.set_fill(color=GRAY, opacity=0.3), run_time=2)
        self.wait(1)  # 等待初始状态

        # 让小球运动一段时间
        run_time = 2  # 设置运行时间
        for _ in range(int(run_time / self.dt)):
            self.wait(self.dt)
        ...
        # 播放最后的静态小球
        final_state = ball.copy()
        self.play(FadeOut(background), Transform(ball, final_state))