...
class CLTAnimation(Scene):
    def construct(self):
        ...
        def set_histogram(densities, animate=False):
            """批量设置所有矩形的高度，animate 决定是否使用动画"""
            animations = []
            for bar, dens in zip(bars, densities):
                h = get_bar_height(dens)
                if animate:
                    animations.append(ApplyMethod(bar.stretch_to_fit_height, h, about_edge=DOWN))
                else:
        ...
        for i in range(1, len(n_values)):
            ...
            # 更新直方图（动画）
            set_histogram(new_dens, animate=True)

            # 更新 n 文本
            new_n_text = Text(f"n = {n}", font_size=28).to_corner(UR)
            self.play(ReplacementTransform(n_text, new_n_text))
            n_text = new_n_text

            # 更新理论曲线
            new_curve = get_theory_curve(n)
            self.play(ReplacementTransform(curve, new_curve))
            curve = new_curve
