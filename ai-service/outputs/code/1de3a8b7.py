...
class MatrixTransformation(Scene):
    def construct(self):
        ...
        # 矩阵显示（初始为单位矩阵，使用Text避免LaTeX编译超时）
        matrix_text = Text(
            "[[1.00, 0.00], [0.00, 1.00]]",
            font_size=30,
            color=WHITE
        ...
        def update_polygon(mob, dt):
            ...
            mob.set_points_as_corners([
                new_points[0], new_points[1], new_points[2], new_points[3], new_points[0]
            ])
        ...
        def update_matrix(mob, dt):
            ...
            a, b = M[0, 0], M[0, 1]
            c, d = M[1, 0], M[1, 1]
            # 直接修改Text对象的字符串，避免频繁LaTeX编译
            mob.text = "[[{:.2f}, {:.2f}], [{:.2f}, {:.2f}]]".format(a, b, c, d)
        ...
        # 添加到场景
        self.add(polygon, dots, matrix_text)
