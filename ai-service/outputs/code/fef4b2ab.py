...
class FourierEighthNote(Scene):
    def construct(self):
        ...
        # 重新排序：先0，再±1，±2，...
        # 从系数中提取频率、振幅、初始相位
        freqs = np.fft.fftfreq(NUM_POINTS, d=1.0)  # 频率（周期数）
        # 选择幅度最大的前 NUM_COMPONENTS 个（排除直流）
        amps = np.abs(coeffs)
        ...
        for idx in selected_indices:
            ...
            if amp < 1e-5:
                continue
            phase = np.angle(c)
            freq = int(round(freqs[idx].real))   # 频率（整数周期数）
        ...
        # 末端追踪点（初始在最后位置）
        trace_point = Dot(current_pos, color=GREEN, radius=0.08, glow=True)
        # 轨迹
        ...
        dummy_clock = Mobject()
        dummy_clock.add_updater(lambda m, dt: pos_tracker.increment_value(dt))  # 只在时间上递增
